...
# ╔═╡ 6ac19326-157f-4300-9d4e-d262cfb43d57
using DataStructures

# ╔═╡ 7949a4ee-002a-4afb-9f66-261cf48a1faf
begin
	using JuMP, HiGHS
end
...
# ╔═╡ ba384152-98b3-473a-8854-558fb5c6e60d
struct Machine
	diagram::String
	schematics::Vector{Vector{Int64}}
	joltage::Vector{Int64}
end

# ╔═╡ 25c1db54-f713-416e-bee4-efc299a9bac6
function parse_input(input_str::String)::Vector{Machine}
	map(
		line -> begin
			parts = match(r"\[([.#]+)\] (.*) {([\d,]+)}", line)
			Machine(
				parts[1],
				map(
					x -> parse.(Int64, split(x[1], ",")),
					eachmatch(r"\(([\d,]+)\)", parts[2])
				),
				parse.(Int64, split(parts[3], ","))
			)
		end,
...
# ╔═╡ 2865ca52-dd5b-450b-964f-7fec7f1dd642
...
# ╔═╡ 00185581-d47c-4a9c-b697-97e511a708b3
function make_mask(nodes::Vector{Int64})::Int64
	reduce(
		(acc, val) -> acc | (1 << val),
		nodes,
		init=0
...
# ╔═╡ 413ec6bd-1006-466f-9add-17b2913767de
function solve_machine(machine::Machine)::Number
	goal_mask = make_mask(findall('#', machine.diagram).- 1)
	button_masks = map(make_mask, machine.schematics)
    queue = Queue{Tuple{Int64, Int64}}()
	push!(queue, (0, 0))
    visited = Set([0])

    while !isempty(queue)
        (curr, steps) = popfirst!(queue)
        if curr == goal_mask
            return steps
		end

        for button_mask in button_masks
            nxt = xor(curr, button_mask)
            if ! (nxt in visited)
                push!(visited, nxt)
				push!(queue, (nxt, steps + 1))
			end
		end
	end
    return 0
...
function solve_part1(input::Vector{Machine})::Number
	return sum(map(solve_machine, input))
end
...
# ╔═╡ 885ebbe5-5342-4e5c-a294-9a1d058ac674
...
# ╔═╡ 394caf2e-d711-4792-9fa2-7399e54c586a
function solve_machine_p2(machine::Machine)::Number
    goal_counters = machine.joltage
    buttons_masks = map(make_mask, machine.schematics)
    num_goals = length(goal_counters)
    num_buttons = length(buttons_masks)

    shifts = collect(0:(num_goals-1))
    
    constraint_matrix = float.(
        (reshape(buttons_masks, 1, num_buttons) .>> reshape(shifts, :, 1)) .& 1
    )

    target_vector = float.(goal_counters)

    model = Model(HiGHS.Optimizer)
    set_silent(model)
    @variable(model, x[1:num_buttons] >= 0, Int)
    @objective(model, Min, sum(x))
    @constraint(model, constraint_matrix * x .== target_vector)

    optimize!(model)

    if termination_status(model) == MOI.OPTIMAL
        return Int64(round(objective_value(model)))
    else
        return NaN
    end
end
...
function solve_part2(input::Vector{Machine})::Number
	return sum(
		map(
			solve_machine_p2,
			input
...
manifest_format = "2.0"
project_hash = "27b6dd6890cb63e967f68aafe375d21004a76103"
