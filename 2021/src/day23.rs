...
fn get_input<const ROOM_SIZE: usize>(input: &str) -> Cave<ROOM_SIZE> {
  let mut s = input
      .lines()
...
fn solve<const ROOM_SIZE: usize>(cave: &Cave<ROOM_SIZE>) -> Cost {
  ...
  candidates.push(Element::new(0, cave.clone()));
